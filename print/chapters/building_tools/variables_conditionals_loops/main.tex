...
start to gain the knowledge necessary to build such tools.
...
Verify that \(p(n)\) is \index{prime} for \(n\in \mathbb{Z}\) up until \(n=20\).

\item 

What is the smallest value of \(n\) for which \(p(n)\) is no longer \index{prime}?
...
You will use \texttt{sympy} to check if a number is \index{prime}.
...
list of boolean variables that confirm if \(p(n)\) is \index{prime}.
...
different things.
This is done using what is called an \texttt{if} statement. Following an
\texttt{if}, a boolean
...
To repeat code while a condition holds a \texttt{while} loop should be used. Similarly to
...
of \(n\) for which the absolute value of that polynomial is not \index{prime}?

\item 

How many \textbf{unique} primes up until the first non \index{prime} value are there?
